	...
	exports.default = {
	  ...
	  watch: {
	    ...
	    val: function val(_val, old) {
	      ...
	      if (_val || _val && _val.length) {
	        // 如果用户传入的是简单string数组或string，则进行一次封装
	        if (_val instanceof Array && _val[0][this.optionsValue] === undefined) {
	          this.val = _val.map(function (val) {
	            return {
	              label: val,
	              value: val
	            };
	          });
	        } else if (_val[this.optionsValue] === undefined) {
	          this.val = {
	            label: _val,
	            value: _val
	          };
